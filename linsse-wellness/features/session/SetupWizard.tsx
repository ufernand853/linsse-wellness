...
export function SetupWizard({ config, stage, reducedMotion, onChange, onBack, onStart, onResult }: Props) {
  ...
  const savePlace = () => localStorage.setItem("linsse-my-place", JSON.stringify({ environment: config.environment, mode: config.mode, duration: config.duration, bilateralSpeed: config.bilateralSpeed }));
